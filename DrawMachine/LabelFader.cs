...
namespace DrawMachine
{
        ...
        public void doFade(int speedMs = 40)
        {
            ...
            if (this.colorSteps.Count > 0)
            {
                tm.Tick += (sa, aea) =>
                {
                    ...
                    this.label.ForeColor = System.Drawing.Color.FromArgb(thisStep.R, thisStep.G, thisStep.B);
                    step++;
                    if (step >= end)
                    {
                        tm.Stop();
    ...
    class ColorStep
    {
        ...
        public ColorStep(System.Drawing.Color from)
        {
            this.setFromColor(from);
        }

        public void setFromColor(System.Drawing.Color from)
        {
            this.R = from.R;
            this.G = from.G;
            this.B = from.B;
